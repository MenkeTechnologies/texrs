...
\def\@settopoint#1{\divide#1\p@\multiply#1\p@}
%
% ---- \setlength and \addtolength: NOT ported yet --------------------------
% latex.ltx:10253-10254 are two lines and nothing else:
%
%     \def\setlength#1#2{#1 #2\relax}
%     \def\addtolength#1#2{\advance#1 #2\relax}
%
% An assignment and an \advance through the register the argument names -- and
% every length the standard classes name IS a register now, so the port was
% tried. It is `prelude.tex's stand-in that is still here, and the reason is
% the UNIT rather than the macro.
%
% `em' and `ex' are read off the CURRENT FONT: tex.web §455 takes the quad and
% the x-height out of `font_info', this engine has no current font in the
% expander, and `src/dimen.rs' carries neither unit. article.cls writes six --
% `\setlength\labelsep{.5em}', `\setlength\leftmargini{2.5em}' and four more --
% and size10.clo:88 writes a seventh. With the real \setlength in place the
% class stops at `package article is not loadable: Illegal unit of measure (pt
% inserted)', which is EARLIER in the file than where the stub leaves it. So
% the two lines wait on the unit; BUGS.md records `em'/`ex' as absent.
%
