...
\newcounter{enumiv}
\newcounter{Hfootnote}
% The three counters LaTeX starts at something other than nought. These used to
% be WRITTEN AND NOT IN EFFECT: `Lowerer::preload' kept the definitions a
% preamble makes and DROPPED the commands it emitted, and a counter is a
% register, so every assignment here was one of the dropped ones -- \pageref
% answered 0 where LaTeX answers 1. `preload' keeps the slot writes now, so
% these are in effect and the `.aux' this run leaves carries the page a real
% LaTeX carries: `\newlabel{sec:a}{{1}{1}...}', measured against LaTeX2e
% 2025-11-01 on the same document.
\setcounter{page}{1}
...
% \@startsection itself, latex.ltx:17231-17246, routed into the same
% substitution. Its six arguments are the name, the level, the indent, the skip
% above, the skip below and the font -- and article.cls:361 writes
%
%     \newcommand\section{\@startsection {section}{1}{\z@}%
%        {-3.5ex \@plus -1ex \@minus -.2ex}%
%        {2.3ex \@plus.2ex}{\normalfont\Large\bfseries}}
%
% so the class's own \section is what a document calls once the class really
% loads. Only the first argument survives here: the other five describe the
% BOX -- \addvspace, \everypar, \leavevmode, the penalties -- and the star, the
% optional contents entry and the title are all \@sect@'s already.
%
% Without it the class's definition reached an undefined \@startsection and
% stopped the document; \section's own definition above is what a plain file
% with no class still gets.
\def\@startsection#1#2#3#4#5#6{\@sect@{#1}}
% latex.ltx:18305-18307. \@mkboth writes the running heads, and the initial
% page style is \ps@empty, which lets it be \@gobbletwo -- so LaTeX's own
% starting value is the right one for an engine with no running heads at all.
% article.cls:505 calls it from \tableofcontents and \thebibliography.
...
%
% Defined under a name of its own as well, because a CLASS defines this
% environment too and a class is read after the kernel. article.cls:717 opens it
% with `\list{\@biblabel{\@arabic\c@enumiv}}{\settowidth\labelwidth...}', which
% is a list box and everything in its second argument -- the \usecounter, the
% \renewcommand\theenumiv -- is inside that box. There is no \list here and no
% stomach to put one in, so `src/latex.rs' \let's the name back to this one once
% the class has been read. Before it did, `\begin{thebibliography}{9}' stopped
% the document at `! Undefined control sequence \list.'
\def\@texrs@thebibliography#1{\setcounter{enumiv}{0}\def\theenumiv{\arabic{enumiv}}}
...
\def\tablename{Table}
% latex.ltx:9411-9412 is
%
%     \DeclareRobustCommand{\nobreakspace}{\leavevmode\nobreak\ }
%
% -- a space that a line may not break at. There is no penalty list in the
% mouth and \leavevmode is the stomach's, so what is left is the space itself.
% It is what `~' means, and book.cls:542 writes it between a figure's name and
% its number: `\def\fnum@figure{\figurename\nobreakspace\thefigure}'. Undefined,
% that definition stopped every document with a \caption in it the moment the
% class began loading far enough to make it.
\def\nobreakspace{\ }
% The two the kernel makes, under names of their own as well: article.cls:492
% and book.cls:542 define both, and their version reads the counter through
% \thefigure -- which is \@arabic, which is \number, which a --text run cannot
% carry (see `src/latex.rs'). \@ctrtotext is the door that run has, so
% `src/latex.rs::RECLAIMED' takes these back once the class has been read.
\def\@texrs@fnum@figure{\figurename\ \@ctrtotext{figure}}
\def\@texrs@fnum@table{\tablename\ \@ctrtotext{table}}
...
%
% ---- \@defaultunits -------------------------------------------------------
% latex.ltx:10537 and 12678, both carried word for word:
%
%     \def\@defaultunits{\afterassignment\remove@to@nnil}
%     \def\remove@to@nnil#1\@nnil{}
%
% It reads an assignment whose right-hand side may or may not carry a unit
% already. The caller writes the default unit AFTER the value and closes with
% `\relax\@nnil', so `\@defaultunits\@tempdimb 12pt\relax\@nnil' scans `12pt',
% stops at the `\relax', and \remove@to@nnil -- pushed back by
% \afterassignment the moment the assignment finishes (tex.web §1269) --
% swallows `pt\relax' as its undelimited argument and the \@nnil as its
% delimiter. Given `10' instead, the same line scans `10pt' and the gobbler
% takes only the `\relax'.
%
% \@nnil is already here (line 36), as \@for's end marker.
\def\remove@to@nnil#1\@nnil{}
\def\@defaultunits{\afterassignment\remove@to@nnil}
%
% ---- the point sizes ------------------------------------------------------
% latex.ltx:10818-10829, carried word for word. Each is the numeric part of a
% size with no unit on it, which is what \@setfontsize's second and third
% arguments are and what \@defaultunits then puts a `pt' after. size10.clo
% writes `\@setfontsize\normalsize\@xpt\@xiipt' on its first line; without
% these two the argument is an undefined control sequence and not the numbers
% 10 and 12.
\def\@vpt{5}
\def\@vipt{6}
\def\@viipt{7}
...
%
% ---- \baselinestretch -----------------------------------------------------
% latex.ltx:12619. The factor every baselineskip is multiplied by, and the
% first argument \fontsize hands \set@fontsize. article.cls:116 renews it to
% EMPTY once size10.clo has been read, which is not a factor of zero but no
% factor at all: `\baselineskip\baselinestretch\baselineskip' is then the
% identity tex.web §449 makes of an internal dimension standing alone.
\def\baselinestretch{1}
%
% ---- \@setfontsize -------------------------------------------------------
% latex.ltx:14103-14108:
%
...
%
% The chain it opens is four macros long. \fontsize (10527) is
%
%     \DeclareRobustCommand\fontsize[2]{\set@fontsize\baselinestretch{#1}{#2}}
%
% \set@fontsize (12585-12601) reads the two sizes into \@tempdimb and
% \@tempskipa, \edef's \f@size, \f@baselineskip and \f@linespread out of them,
% and BUILDS \size@update rather than running it:
%
%     \def\size@update{%
...
%
% \selectfont calls it at 12581, one line after the `selectfont' hook.
%
% COLLAPSED into \@setfontsize. What the three intervening links hold is:
%
%   * \f@size and \f@baselineskip, two \edef'd STRINGS. Nothing here reads
%     either -- they name the font \pickup@font would load, and there is no
%     \pickup@font -- and `src/lower.rs::edef_snapshot' cannot freeze a
%     DIMENSION, so `\edef\f@size{\strip@pt\@tempdimb}' stops the load with
%     `Unsupported \edef body: \the\@tempdimb'. Measured, on this document.
%     \strip@pt is not ported for the same reason: its only caller would be
%     that line.
%
%     The refusal is now only about dimensions. \the of an internal INTEGER
%     freezes (a catcode, a \chardef constant, \numexpr) and so does \the of a
%     token register; what is missing for this one is that a dimension's value
%     is a VM slot at run time rather than a table the lowerer holds, and
%     \strip@pt wants the CHARACTERS `12.0pt' at definition time so its
%     \rem@pt can match `#1.#2PT' against them.
%   * \size@update's DEFERRAL, whose point is that a size change inside a group
%     assigns \baselineskip once, when \selectfont finally runs. \selectfont is
%     a stand-in (`prelude.tex'), so there is no later point to defer to and
%     the assignment happens where the size is named.
%   * \strutbox, which is a box; \@nomath, which is a math-mode warning this
%     engine has no \ifmmode to raise; and \@currsize, which only \@setfontsize
%     itself ever writes.
%
% The arithmetic is 12586-12595 word for word, reading the two registers
% \@defaultunits has just filled in place of the strings \edef'd from them,
% and \baselinestretch in place of \f@linespread -- which is the macro
% \fontsize took that argument from to begin with. \@tempdimb is still
% written, because \set@fontsize really does clobber it and size10.clo:110
% assigns it again straight afterwards.
%
% It is the last thing between article.cls and a load. size10.clo's first
% definition is `\renewcommand\normalsize{\@setfontsize\normalsize\@xpt\@xiipt
% ...}', its line 54 calls it, and its line 134 is
% `\divide\@tempdima\baselineskip'. With \@setfontsize empty that divisor
% stayed at INITEX's zero and the class stopped on `Arithmetic overflow'.
\def\@setfontsize#1#2#3{%
  \@defaultunits\@tempdimb#2pt\relax\@nnil
  \@defaultunits\@tempskipa#3pt\relax\@nnil
  \baselineskip\@tempskipa\relax
  \baselineskip\baselinestretch\baselineskip
  \normalbaselineskip\baselineskip}
\def\@setsize#1#2#3#4{\@setfontsize#1{#4}{#2}}
%
% ---- \@settopoint ---------------------------------------------------------
% latex.ltx:10261, carried WORD FOR WORD:
%
%     \def\@settopoint#1{\divide#1\p@\multiply#1\p@}
%
% It rounds a dimension DOWN to a whole point, and it does it with nothing but
% the two arithmetic primitives. \p@ is `\newdimen\p@ \p@=1pt' (latex.ltx:486),
% so both operands are a dimension register standing where a NUMBER is wanted:
% tex.web §430 coerces it to its count of scaled points, which is 65536. The
% division throws the fraction away (§1236 truncates) and the multiplication
% puts the points back.
%
% Nothing is substituted because nothing had to be. What it needed was the
% coercion -- `src/lower.rs::peek_int_register' -- and §407's backing up of a
% failed `by', without which the second `\p@' of `\divide#1\p@' was read twice.
%
% It is what stood between article.cls and a load once the four display skips
% were declared: `texrs: package article needs \@settopoint'. size10.clo calls
% it five times, on \textwidth, \oddsidemargin, \marginparwidth,
% \evensidemargin and \topmargin.
\def\@settopoint#1{\divide#1\p@\multiply#1\p@}
%
% ---- \setlength and \addtolength ------------------------------------------
% latex.ltx:10253-10254 are two lines and nothing else, carried word for word:
%
%     \def\setlength#1#2{#1 #2\relax}
%     \def\addtolength#1#2{\advance#1 #2\relax}
%
% An assignment and an \advance through the register the argument names. Every
% length the standard classes name IS a register now, declared below, so the
% two lines say all they say in latex.ltx.
%
% What held them back was the UNIT and not the macro. `em' and `ex' are read
% off the current font (tex.web §455 takes the quad and the x-height out of
% `font_info'), `src/dimen.rs' carried neither, and article.cls writes six --
% `\setlength\labelsep{.5em}', `\setlength\leftmargini{2.5em}' and four more --
% with a seventh at size10.clo:88. Both units now resolve against cmr10's real
% \fontdimen6 and \fontdimen5, so the stand-in in `prelude.tex' is gone and
% these are what a class file calls.
\def\setlength#1#2{#1 #2\relax}
\def\addtolength#1#2{\advance#1 #2\relax}
%
% ---- \raggedbottom and \flushbottom ---------------------------------------
% latex.ltx:18328-18331, carried word for word:
%
...
%
% Both only DEFINE two names the page builder would read, and a macro body is
% not expanded where it is written -- so \vskip being a primitive this engine
% has no vertical list for costs nothing here. article.cls calls one of the two
% on its own last lines, whichever way \if@twoside went.
\DeclareRobustCommand\raggedbottom{%
  \def\@textbottom{\vskip \z@ \@plus.0001fil}\let\@texttop\relax}
\DeclareRobustCommand\flushbottom{%
  \let\@textbottom\relax \let\@texttop\relax}
%
% ---- \onecolumn, \twocolumn and where floats may go -----------------------
% latex.ltx:20430-20437 and 20454-20465, carried word for word:
%
%     \def\onecolumn{%
%       \clearpage
...
%       \@floatplacement}
%
% article.cls's LAST line of code is one of these two, chosen by \if@twocolumn,
% and \onecolumn was the name every document that loaded article stopped on.
% All seven steps are assignments to registers and switches this file declares,
% except two.
%
% \clearpage is the first. It is `prelude.tex's stand-in, because the real one
% (latex.ltx:20414-20426) ends a page: \newpage, then \@@write, then \penalty
% -\@Mi into a vertical list, and the page builder that would read them is what
% `src/page.rs' does from the finished paragraph list rather than from a
% \penalty a macro contributed. Nothing \onecolumn does depends on the page
% having been flushed, so the stand-in costs this call nothing.
%
% \@floatplacement is the second, latex.ltx:17557-17564, and it is carried
% whole below.
%
% \twocolumn is here with it because the pair is one decision and the register
% values only mean anything against each other -- \onecolumn's \columnwidth is
% \textwidth, \twocolumn's is half of what is left after \columnsep. Its last
% line reads ahead for an optional argument: with a `[' it hands the argument
% to \@topnewpage, which is float machinery this engine has none of, and
% without one it ends on \@floatplacement exactly as \onecolumn does.
% article.cls's own call writes no bracket, so `\documentclass[twocolumn]' and
% plain `\documentclass' reach the same last step -- measured, both stop at
% `package article is not loadable: Illegal unit of measure (pt inserted)'.
% A document that writes \twocolumn[...] itself would stop on \@topnewpage BY
% NAME, which is the right way to find a missing macro.
\def\onecolumn{%
  \clearpage
  \global\columnwidth\textwidth
  \global\hsize\columnwidth
...
% latex.ltx:17557-17573. How many floats a column may take and how much room
% they may have, recomputed whenever the column count changes. Both are carried
% word for word.
%
% Every line is `<register> <factor><register>' or `<count> <counter>': the
% fractions are the macros article.cls:122-130 renews -- \topfraction is `.7',
% \floatpagefraction is `.5' -- and \c@topnumber and its three fellows are the
% count registers article.cls:121-128 would set. \@colht is zero here (see its
% declaration below), so the three room registers are zero whatever fraction is
% taken of them; the four counts are the numbers latex.ltx and article.cls both
% ask for, and `\global\@topnum\c@topnumber' really writes 2.
%
% This is as far as the article load goes today, and the second line is where
% it stops: `\topfraction\@colht' is tex.web S455's `<factor><internal unit>'
% with the factor arriving from a MACRO, and `Engine::scan_factor'
% (`src/expand.rs') reads the coefficient without expanding, so the control
% sequence reaches the unit scan and is answered `! Illegal unit of measure (pt
% inserted).' Written out -- `\@toproom .7\@colht' -- the same line scans. The
% six lines are left as latex.ltx writes them rather than being forced through
% \expandafter, because the gap is the scanner's and every
% `\setlength{\a}{\somefraction\b}' in LaTeX has it.
...
% latex.ltx:22007-22009. The three penalties \@getpen picks from, which
% article.cls sets on its own lines 118-120 and reads back into the list
% penalties above.
\newcount\@lowpenalty
\newcount\@medpenalty
\newcount\@highpenalty
% latex.ltx:15748-15749. The glue \eqnarray sets \tabskip to, which is what
% centres a column: infinitely stretchable either side of it. amsmath.sty:66
% is `\@mathmargin\@centering', six lines into the package, and without the
% declaration the skip it copies from is not a register at all -- the scan
% reaches a name where a unit belongs and answers `! Illegal unit of measure'.
\newskip\@centering
...
\newtoks\everymath
\newtoks\everydisplay
% tex.web §224 makes these GLUE parameters of the engine, so latex.ltx never
% declares them and every class assigns to them as though they were always
% there -- size10.clo's \normalsize opens
%
%     \abovedisplayskip 10\p@ \@plus2\p@ \@minus5\p@
%
% and every size option of every standard class writes the same four lines.
% texrs has no glue parameters and it does have glue registers, so they are
% declared here exactly as \everypar above is: the store the engine really has,
% under the name every file assumes. INITEX starts them at zero and the class
% file is what sets them, so no value is written here either.
%
% Undeclared, the four display skips were the FIRST thing between article.cls
% and a load: `texrs: package article needs \abovedisplayskip' on the simplest
% document there is. \baselineskip is the next one of them the same file wants,
% at size10.clo:134 --
%
%     \divide\@tempdima\baselineskip
%
% which is a glue parameter standing where a NUMBER is wanted (§430's coercion,
% `src/lower.rs::peek_int_register'), so the divisor is the natural width in
% scaled points. As an empty macro it divided by nothing at all.
...
\newskip\parskip
\newskip\topskip
% tex.web §247's DIMENSION parameters, declared for the same reason and in the
% same way. size10.clo:88-100 sets the first two -- `\setlength\parindent{15\p@}'
% and `\setlength\maxdepth{.5\topskip}' -- article.cls:87 sets \overfullrule from
% its draft option, and \lineskiplimit is what plain's \normalbaselines restores.
% \hoffset and \voffset are here because a document sets them itself.
%
% \hsize is the same kind of name and is here for the same reason. It is the
% measure a paragraph is broken to (tex.web §247), latex.ltx never declares it
% because INITEX already has it, and \onecolumn and \twocolumn both assign to
% it on article.cls's last lines -- `\global\hsize\columnwidth'. texrs decides
% the measure in `src/typeset.rs::Layout' from the class options rather than
% from this register, so what is carried here is the STORE and not the effect:
% the assignment succeeds and \the\hsize answers what was written, and the
% paragraph is still broken to the layout's measure. BUGS.md's note about one
% type size covers the same ground.
\newdimen\parindent
\newdimen\maxdepth
...
\newdimen\voffset
\newdimen\hsize
% tex.web §247's last dimension parameter here, and the one \setlength made
% necessary: a Pandoc preamble writes `\setlength{\emergencystretch}{3em}' and
% `src/latex/load.rs::AFTERWARDS' makes every package prove that line still
% runs after it. The stand-in in `prelude.tex' was delimited on an `=' and had
% nothing to match.
\newdimen\emergencystretch
% plain.tex:378-383. The skip amounts \smallskip, \medskip and \bigskip are
% named for, and the three \normalbaselines restores from.
...
\newskip\normallineskip
\newdimen\normallineskiplimit
% latex.ltx:20362-20381. The page dimensions, declared there with \newdimen and
% set by the size option's .clo -- size10.clo computes all of \textwidth,
% \textheight, \oddsidemargin, \evensidemargin, \marginparwidth and \topmargin
% from \paperwidth and \paperheight. A class that found these undefined stopped
% on the first \setlength: `texrs: package article is not loadable: Unsupported
% register \textwidth'.
\newdimen\topmargin
...
\newdimen\stockheight
\newdimen\stockwidth
% latex.ltx's other lengths that article.cls and size10.clo assign to:
% the footnote rule's space (17648), the float separations (ltfloat), the
% \fbox and tabular rules (ltboxes, ltarray), and \tabbingsep.
\newdimen\footnotesep
\newdimen\arraycolsep
...
\newskip\@dblfpsep
\newskip\@dblfpbot
% latex.ltx:20392-20408. The column count and the float placement registers.
%
% article.cls's last two lines reach all of them: \onecolumn writes the column
% count, and the \@floatplacement it ends on writes six more. They are page
% registers like the block above and are declared with it.
%
% They all start at zero, as INITEX leaves them and as latex.ltx leaves them,
% except the column count: latex.ltx sets it to one on the line below its
% declaration, because a document is one column until \twocolumn says
% otherwise. That one line is NOT here -- it is written as \col@number \@ne
% and \@ne is a \chardef this file makes further down, where plain.tex's
% constants are, so it is carried with them instead. latex.ltx has plain's
% constants long before this block and does not have to choose.
%
% \@colht and \@colroom are the height of the current column, which the output
% routine sets and this engine has no output routine to set -- so what
% \@floatplacement takes a fraction of is zero here, and every room register it
% writes comes out zero with it.
\newcount\col@number
\newcount\@topnum
\newdimen\@toproom
\newcount\@dbltopnum
...
\@mparbottom\z@
\newcount\@currtype
% latex.ltx:21541-21554. How many floats a column may hold and what fraction of
% it they may fill -- the four numbers and the six fractions \@floatplacement
% and \@dblfloatplacement read.
%
% latex.ltx declares the four as COUNT REGISTERS, not as counters: it writes
% the \newcount for \c@topnumber itself rather than \newcounter{topnumber}, so
% there is no \the-form, no reset list and no \p@ prefix for any of them. That
% is carried as written, and it means the register lives in the pool
% `src/latex/allocate.rs' hands out rather than in the counter block above.
%
% SUBSTITUTION, in the line that follows each declaration. latex.ltx sets the
% value with \setcounter, and this layer's \setcounter reaches a counter BY
% NAME through \ctr@NAME (see the counters section above) -- which these four
% have not got, because they were never made counters. So each is set as the
% register assignment \setcounter would have performed.
%
% What that costs is narrow and worth naming: a class that wanted DIFFERENT
% numbers here would not be followed, because its own \setcounter{topnumber}
% reaches nothing either. article.cls:121-128 and every other standard class
% re-state latex.ltx's own defaults -- 2, 1, 3, 2 -- so the values below are
% what a standard class leaves behind in any case.
%
% The fractions are plain macros in latex.ltx and are plain macros here; a
% class renews them by name and that works unchanged.
\newcount\c@topnumber
\c@topnumber 2
...
% latex.ltx:20393, held back from its own block above until \@ne exists. The
% column count is one until \twocolumn says otherwise, and this is latex.ltx's
% line for saying so, token for token.
\col@number \@ne
% plain.tex: \newcount\m@ne \m@ne=-1.
%
...
\def\m@ne{-1}
% latex.ltx:16343 and 17629 allocate the two footnote insertions \@mpfootins
% and \footins, with plain TeX's insertion allocator.
%
% They are written by hand here rather than declared with it, and the NUMBERS
% are the reason. plain's allocator counts DOWN from 255 and reserves the
% \count, \dimen, \skip and \box of that one number together, exactly so that an
% insertion's \skip cannot be a \newskip name's; `src/latex/allocate.rs' has one
% upward counter per pool, so an insertion would have come out in the low teens
% and `\skip\footins' -- which is how size10.clo:203 and article.cls:448 reach
% it -- would have named a glue register \newskip had already given away.
%
% So the two are frozen at the numbers a real LaTeX gives them, measured:
% `\number\footins' is 253 and `\number\@mpfootins' is 254 under LaTeX2e
% 2025-11-01. Both are clear of the upward pool by 240 names.
%
% (The allocator reads the file as TEXT and does not skip comments, so a
% declaration quoted in one of these notes really allocates. That is why the
% two are named above rather than written out.)
\chardef\footins=253
\chardef\@mpfootins=254
